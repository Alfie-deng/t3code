...
| 上次合入上游                                    | 本轮 merge（合入上游 21 笔至 `6bc6cb6be`：PR 面/主题/右侧空态/手机气泡列表与命令层）              |
| 合并基点 merge-base（相对当下 `upstream/main`） | `6bc6cb6be`（已与 `upstream/main` 对齐）                                                          |
| 官方 tip（已 fetch）                            | `6bc6cb6be`                                                                                       |
| 桌面安装版                                      | `/Applications/T3 Code.app` `0.0.33`（合完后按 §6.1 覆盖；装成再改本行）                          |
| 手机安装版                                      | 真机 Alfie iPhone 17 Pro Max · `com.jetdeng.t3code` · `1.0.3`（合完后按 §6.2 重装；装成再改本行） |
