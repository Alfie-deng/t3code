  ...
  return (
    ...
    >
      {ADD_PROVIDER_WIZARD_STEPS.map((step, index) => {
        const stepLabel = translateZhCnUiText(step);
        const stepAriaLabel = translateZhCnUiText(`${step}, step ${index + 1}`);
        return (
          <li key={step} className="min-w-0">
            <button
              ...
              aria-label={`${stepAriaLabel}${index < currentStep && summaries[index] ? `，${summaries[index]}` : ""}`}
              onClick={() =>
                onNavigation(
