...
export function ProviderModelsSection({
  ...
  return (
    <div>
      <div className="text-xs font-medium text-foreground">{translateZhCnUiText("Models")}</div>
      <div className="mt-1 text-xs text-muted-foreground">
        {translateZhCnUiText(`${models.length} model${models.length === 1 ? "" : "s"} available.`)}
      </div>
      <div ref={listRef} className="mt-2 max-h-40 overflow-y-auto pb-1">
        {orderedModels.map((model, index) => {
          ...
          if (descriptors.some((descriptor) => descriptor.id === "fastMode")) {
            capLabels.push(translateZhCnUiText("Fast mode"));
          }
          if (descriptors.some((descriptor) => descriptor.id === "thinking")) {
            capLabels.push(translateZhCnUiText("Thinking"));
          }
          if (
            descriptors.some(
              (descriptor) =>
            ...
            capLabels.push(translateZhCnUiText("Reasoning"));
          }
              ...
              <div className="flex min-w-0 items-center gap-1">
                    ...
                    <TooltipTrigger
                      render={
                        <Button
                          ...
                          aria-label={translateZhCnUiText(`Details for ${model.name}`)}
                        />
                      }
                    >
                ...
                {isHidden ? (
                  <span className="text-[10px] text-muted-foreground">
                    {translateZhCnUiText("hidden")}
                  </span>
                ) : null}
                {model.isCustom ? (
                  <span className="text-[10px] text-muted-foreground">
                    {translateZhCnUiText("custom")}
                  </span>
                ) : null}
              </div>
              <div className="flex shrink-0 items-center gap-0.5">
                <Tooltip>
                  <TooltipTrigger
                    render={
                      <Button
                        ...
                        aria-label={translateZhCnUiText(
                          `${isFavorite ? "Remove" : "Add"} ${model.name} ${
                            isFavorite ? "from" : "to"
                          } favorites`,
                        )}
                      />
                  ...
                  <TooltipPopup side="top">
                    {translateZhCnUiText(isFavorite ? "Remove from favorites" : "Add to favorites")}
                  </TooltipPopup>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger
                    render={
                      <Button
                        ...
                        aria-label={translateZhCnUiText(`Move ${model.name} up`)}
                      />
                    }
                  >
                    <ArrowUpIcon className="size-3" />
                  </TooltipTrigger>
                  <TooltipPopup side="top">{translateZhCnUiText("Move up")}</TooltipPopup>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger
                    render={
                      <Button
                        ...
                        onClick={() => handleMove(model.slug, 1)}
                        aria-label={translateZhCnUiText(`Move ${model.name} down`)}
                      />
                    }
                  >
                    <ArrowDownIcon className="size-3" />
                  </TooltipTrigger>
                  <TooltipPopup side="top">{translateZhCnUiText("Move down")}</TooltipPopup>
                </Tooltip>
                {!model.isCustom ? (
                  <Tooltip>
                    <TooltipTrigger
                    ...
                    <TooltipPopup side="top">
                      {translateZhCnUiText(isHidden ? "Show in picker" : "Hide from picker")}
                    </TooltipPopup>
                  </Tooltip>
                ) : null}
                {model.isCustom ? (
                  <Tooltip>
                    <TooltipTrigger
                      render={
                        <Button
                          ...
                          aria-label={translateZhCnUiText(`Remove ${model.slug}`)}
                    ...
                    <TooltipPopup side="top">
                      {translateZhCnUiText("Remove custom model")}
                    </TooltipPopup>
                  </Tooltip>
                ) : null}
              </div>
      ...
      <div className="mt-3 flex flex-col gap-2 sm:flex-row">
        ...
        <Button className="shrink-0" variant="outline" onClick={handleAdd}>
          ...
          {translateZhCnUiText("Add")}
        </Button>
      </div>

      {error ? <p className="mt-2 text-xs text-destructive">{translateZhCnUiText(error)}</p> : null}
    </div>
  );
}
