    ...
    <SidebarInset className="h-dvh min-h-0 overflow-hidden overscroll-y-none bg-background text-foreground isolate">
      <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-background text-foreground">
        {!isElectron && (
          ...
          >
            <WorkspaceBreadcrumb ariaLabel="用量路径">
              <WorkspaceBreadcrumbItem current>用量</WorkspaceBreadcrumbItem>
            </WorkspaceBreadcrumb>
          </header>
        )}
              ...
              <>
                  ...
                  <div className="flex flex-col gap-5">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {metric === "cost"
                          ? "* 按完整 API 费率计费时的估算值"
                          : `涵盖 ${formatCount(merged.sessions)} 个会话的输入、缓存读取和输出。`}
                      </span>
                    </div>
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <h2 className="text-sm font-medium text-foreground">
                        {isPast24Hours ? "每小时" : "每日"}
                        {metric === "tokens" ? "消耗的 Token" : "成本"}
                      </h2>
                ...
                <section className="grid grid-cols-2 gap-px border-y border-border bg-border md:grid-cols-5">
                  <Metric
                    ...
                    detail={`活跃${isPast24Hours ? "小时" : "日"}均 ${formatTokens(periodAverage)}`}
                  />
                  <Metric
                    label="已缓存输入"
                ...
                <section className="flex flex-col gap-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex overflow-hidden rounded-md border border-border">
                      {(
                        ...
                        >
                          {option.value === "model" ? "模型" : isPast24Hours ? "小时" : "日期"}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {breakdown === "model" ? (
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-border text-left text-xs text-muted-foreground">
                          <th className="py-2 font-normal">{isPast24Hours ? "小时" : "日期"}</th>
