...
export const ChatHeader = memo(function ChatHeader({
  ...
  return (
    ...
    >
      <WorkspaceBreadcrumb ariaLabel="线程路径" className="flex-1">
              ...
              <Tooltip>
                <TooltipTrigger
                  render={
                    <button
                      type="button"
                      aria-label={`在 ${activeProjectName} 中新建线程`}
                ...
                <TooltipPopup side="top">在 {activeProjectName} 中新建线程</TooltipPopup>
              </Tooltip>
            </WorkspaceBreadcrumbItem>
            <WorkspaceBreadcrumbSeparator />
          </>
        ) : null}
        <WorkspaceBreadcrumbItem current className="min-w-0 flex-none">
          {renamingTitle !== null ? (
            <input
              autoFocus
              aria-label="线程标题"
              ...
              <TooltipTrigger
                render={
                  <button
                    ref={titleButtonRef}
                    type="button"
                    aria-label={`线程操作：${activeThreadTitle}`}
                    aria-haspopup="menu"
                    onClick={openMenuFromTitle}
                    className="group/thread-title inline-flex min-w-0 max-w-full flex-none cursor-pointer items-center gap-1 rounded-sm text-left focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring"
                  />
                }
              >
                <h2 className="min-w-0 max-w-full truncate">{activeThreadTitle}</h2>
                <ChevronDownIcon
                  aria-hidden
              ...
              <TooltipTrigger
                render={
                  <h2
                    ...
                    className="min-w-0 max-w-full flex-none truncate"
                  >
                    {activeThreadTitle}
                  </h2>
                }
