...
function DisclosureRow(props: {
  ...
  return (
    ...
    >
      <Text className="text-sm font-t3-medium text-foreground">{t(props.label)}</Text>
      <View className="flex-1" />
      {props.value ? (
  ...
  return (
    ...
    >
      <View className="min-w-0 flex-1 gap-0.5">
        <Text className="text-base font-t3-medium text-foreground">{t(props.label)}</Text>
        {props.description ? (
          <Text className="text-sm leading-5 text-foreground-muted">{t(props.description)}</Text>
        ) : null}
      </View>
      {props.selected ? (
        <SymbolView
