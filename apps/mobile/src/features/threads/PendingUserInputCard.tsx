  ...
  const bar = showBar ? (
    ...
    >
      <Pressable
        ...
        accessibilityLabel={
          questionCount === 1
            ? t("Expand user input, 1 question")
            : t("Expand user input, {count} questions").replace("{count}", String(questionCount))
        }
      ...
      >
        ...
        <Text className="font-sans text-xs text-neutral-500 dark:text-neutral-400">
          {questionCount === 1
            ? t("1 question")
            : t("{count} questions").replace("{count}", String(questionCount))}
        </Text>
