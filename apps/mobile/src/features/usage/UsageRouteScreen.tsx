    ...
    <View className="gap-4 rounded-[24px] border-continuous bg-card p-4">
      <View className="flex-row items-start justify-between gap-3">
        <View className="min-w-0 flex-1 gap-0.5">
          ...
          <Text className="text-sm text-foreground-muted">
            {metric === "cost"
              ? "* 按完整 API 费率计费时的估算值"
              : `涵盖 ${formatCount(merged.sessions)} 个会话`}
          </Text>
        </View>
      ...
      {hasActivity ? (
        ...
        <View style={{ height: CHART_HEIGHT }} className="items-center justify-center">
          <Text className="text-base text-foreground-muted">此时间范围内没有活动。</Text>
        </View>
      )}
...
function ProviderSection(props: {
  ...
  return (
    <SettingsSection title="提供商" card>
...
function TotalsSection(props: { readonly merged: MergedUsage; readonly isPast24Hours: boolean }) {
  ...
  return (
    <SettingsSection title="总计" card>
      <View className="flex-row flex-wrap">
        <MetricCell
          label="已消耗 Token"
          value={formatTokens(merged.totalTokens)}
          detail={`活跃${props.isPast24Hours ? "小时" : "日"}均 ${formatTokens(periodAverage)}`}
        />
        <MetricCell
          label="缓存节省"
        ...
        <MetricCell
          ...
          detail={`新增缓存写入 ${formatTokens(merged.cacheCreationTokens)}`}
        />
        <MetricCell
          label="输出"
...
function ModelsSection(props: { readonly merged: MergedUsage }) {
  ...
  return (
    <SettingsSection title="按模型" card>
      {merged.models.map((model, index) => (
        ...
        >
          ...
          <View className="min-w-0 flex-1 gap-0.5">
            ...
            <Text className="text-sm text-foreground-muted">
              成本占比 {formatPercent(model.costShare)} · {formatTokens(model.totalTokens)} Token
            </Text>
          </View>
    ...
    <View className="gap-1 rounded-[16px] border-continuous bg-card px-4 py-3">
      ...
      {failed.map((environment) => (
        <Text key={environment.environmentId} className="text-sm text-foreground-muted">
          {environment.label} 无法报告用量。
        </Text>
      ))}
      {stale.map((environment) => (
        <Text key={environment.environmentId} className="text-sm text-foreground-muted">
          {environment.label} 使用旧版服务器，已从统计中排除。
        </Text>
      ))}
      {duplicateSources.length > 0 ? (
        <Text className="text-sm text-foreground-muted">
          共用会话记录目录的环境只计一次： {duplicateSources.join(", ")}
        </Text>
      ) : null}
    </View>
  );
